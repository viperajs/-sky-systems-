if SkyDiagnostics then SkyDiagnostics.FileStarted("sky_mechanicjob/source/import.lua") end
if not SkyDiagnostics and not _SKY_LOGGING_UNAVAILABLE_WARNED then
    _SKY_LOGGING_UNAVAILABLE_WARNED = true
    print("^3[sky_mechanicjob][WARN][logging.unavailable] source/diagnostics.lua did not load. Update this resource including fxmanifest.lua and source/diagnostics.lua, then restart it. Native gameplay registration remains enabled.^0")
end
-- =====================================================
--  sky_mechanicjob · source/import.lua
--  Self-Contained Framework Bridge & Runtime Importer
-- =====================================================
...
function Sky.Debug(level, formatStr, ...)
    ...
    local formattedMsg
    if argCount > 0 then
        local ok, res = pcall(string.format, formatStr, unpackFn(args, 1, argCount))
        formattedMsg = ok and res or formatStr
    else
...
function formatColorRgb(col)
    if type(col) ~= "table" then return "nil" end
    return ("(%s,%s,%s)"):format(tostring(col.r or 0), tostring(col.g or 0), tostring(col.b or 0))
end

function saveTuningForVehicle(vehicle)
    if not vehicle or vehicle == 0 or not DoesEntityExist(vehicle) then
        vehicle = (TuningState and TuningState.vehicle) or nil
    end
    if not vehicle or vehicle == 0 or not DoesEntityExist(vehicle) then
    ...
    local props = Sky and Sky.Vehicle and Sky.Vehicle.new(vehicle):GetVehicleProperties()

    if Sky and Sky.Cb and Sky.Cb.Trigger then
        Sky.Cb.Trigger("sky_mechanicjob:tuning:saveProperties", {
            plate = plate,
            properties = props
        })
    else
        TriggerServerEvent("sky_mechanicjob:tuning:saveProperties", {
            plate = plate,
            properties = props
        })
    end

    if StanceKit and StanceKit.PersistCurrent then
        pcall(function() StanceKit.PersistCurrent(vehicle) end)
    end
...
-- -----------------------------------------------------
    ...
    function Sky.Cb.TriggerWithTimeout(name, timeout, ...)
        ...
        while clientCallbackResponses[requestKey] == true do
            ...
            if GetGameTimer() > (startTime + timeout) then
                ...
                Sky.Debug("error", "ServerCallback " .. name .. " timed out after " .. timeout .. "ms!")
                break
            end
        end
    ...
    RegisterNetEvent("sky_base:cc", function(name, requestId, args)
        ...
        if not packed[1] then
            TriggerServerEvent("sky_base:ccError", name .. tostring(requestId), name, tostring(packed[2]))
            return
        end

        table.remove(packed, 1)
        TriggerServerEvent("sky_base:ccResponse", name .. tostring(requestId), packed)
    end)
