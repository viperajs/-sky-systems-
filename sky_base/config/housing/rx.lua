if SkyDiagnostics then SkyDiagnostics.FileStarted("sky_base/config/housing/rx.lua") end
...
local function getRxResourceName()
    if GetResourceState("RxHousing") == "started" then
        return "RxHousing"
    end
    if GetResourceState("rx_housing") == "started" then
        return "rx_housing"
    end
    return nil
end

Sky.Housing.RegisterAdapter("RxHousing", function(source)
    local resourceName = getRxResourceName()
    if not resourceName then
        return nil
    end

    local ok, properties = pcall(function()
        return exports[resourceName]:GetAllProperties()
    end)
    ...
    for _, property in pairs(properties) do
        local propertyId = property and (property.id or property.propertyId or property.property_id)
        local players = property and (property.players or property.playersinside or property.playersInside)

        if type(players) ~= "table" and propertyId then
            ok, players = pcall(function()
                return exports[resourceName]:GetPlayersInProperty(propertyId)
            end)
            if not ok then
                players = nil
            end
        ...
        if type(players) == "table" then
            for _, player in pairs(players) do
                if player == sourceNumber or tostring(player) == sourceString or tonumber(player and player.source) == sourceNumber then
