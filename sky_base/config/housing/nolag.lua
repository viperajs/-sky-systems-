if SkyDiagnostics then SkyDiagnostics.FileStarted("sky_base/config/housing/nolag.lua") end
...
Sky.Housing.RegisterAdapter("nolag_properties", function(_, fallbackCoords)
    if IsDuplicityVersion() or GetResourceState("nolag_properties") ~= "started" then
        return nil
    end

    local ok, property = pcall(function()
        return exports.nolag_properties:GetCurrentProperty()
    end)
    if ok and type(property) == "table" then
        local coords = Sky.Housing.ResolvePropertyEntrance(property)
        if coords then
            return coords
        end
    end
    ...
    local coords
    ok, coords = pcall(function()
        return exports["nolag_properties"]:GetEntryCoordinates()
    end)
    if not ok then
        return nil
    end

    return Sky.Housing.CoordsChanged(coords, fallbackCoords)
end)
--   ____  _____ ____ ______   ______ _____ _____ ____     ___     _____ _____  _______ ____    ______   __  _______  __    _    ____    _____ ____  ___ _  _____ _____ ___  _   _    _        _     _   _                  ____  _ _                       _                 _______ _____ _  __     _   _ ____      _ ____ _____ _____
--  |  _ \| ____/ ___|  _ \ \ / /  _ \_   _| ____|  _ \   ( _ )   |  ___|_ _\ \/ / ____|  _ \  | __ ) \ / / |  ___\ \/ /   / \  |  _ \  |  ___|  _ \|_ _| |/ /_ _|_   _/ _ \| \ | |  / \      | |__ | |_| |_ _ __  ___ _   / / /_| (_)___  ___ ___  _ __ __| |  __ _  __ _   / / ____|_   _| |/ /__ _| | | | ___|  __| | ___|___  |___  |
--  | | | |  _|| |   | |_) \ V /| |_) || | |  _| | | | |  / _ \/\ | |_   | | \  /|  _| | | | | |  _ \\ V /  | |_   \  /   / _ \ | |_) | | |_  | |_) || || ' / | |  | || | | |  \| | / _ \     | '_ \| __| __| '_ \/ __(_) / / / _` | / __|/ __/ _ \| '__/ _` | / _` |/ _` | / /|  _|   | | | ' // _` | |_| |___ \ / _` |___ \  / /   / /
